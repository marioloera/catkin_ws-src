...
float v=0.0;
float w=0.0;
float wr,wl,wr_hat,wl_hat,v_hat,vmin;
int const ticks=360;
int const freq=10;
float const base=0.23;
float const radius=0.0352;
float pwm_delta=0;
float ki_l=8.20*1;//8.2
float ki_r=7.47*1;//7.47
...
void encodersCallback(const ras_arduino_msgs::Encoders::ConstPtr& encoder_msg)
{
   ...
   v_hat=(double)(wl_hat+wr_hat)*radius/2;
   //ROS_INFO("hear: enc1=%3d enc2=%3d v_hat=%.4f wl_hat=%.4f wr_hat=%.4f", delta_encoder1,delta_encoder2,v_hat,wl_hat,wr_hat);
   ROS_INFO("v_hat=%.4f wl_hat=%.4f wr_hat=%.4f",v_hat,wl_hat,wr_hat);
...
//////////////////////////////////////////////////////////////////////////////////
    vmin=v;
    wl=(double) (vmin/radius)-(base*w)/(2*radius);
    wr=(double) (vmin/radius)+(base*w)/(2*radius);
   


 
    pwm_delta=(double) ki_l*(wl-wl_hat);
    pwm1=(int) pwm1+pwm_delta;
    pwm_delta=(double) ki_r*(wr-wr_hat);
...
// %Tag(ROSCONSOLE)%
    ROS_INFO("wl=%.3f wr=%.3f",wl,wr);
    ROS_INFO("Loop=%5d - pwm1=%4d - pwm2=%4d",count,pwm1,pwm_msg.PWM2);//print in terminal
//    ROS_INFO("absolute value of -10.48 is: %f",fabs(-10.48));//print in terminal
